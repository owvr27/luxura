...
class MemoryCache {
  ...
  set<T>(key: string, value: T, ttl: number = 300000): void { // Default 5 minutes TTL
...
export function createCacheMiddleware(options: CacheOptions = {}) {
  const {
    keyGenerator = (req: any) => `${req.method}:${req.originalUrl}:${JSON.stringify(req.query)}`,
...
export function Cacheable(ttl: number = 300000, keyPrefix?: string) {
...
export async function warmCache(warmers: CacheWarner[]): Promise<void> {
