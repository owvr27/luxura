...
export class GamificationService {
  ...
  async getLeaderboard(limit: number = 10, offset: number = 0): Promise<LeaderboardEntry[]> {
    try {
      ...
      const leaderboard: LeaderboardEntry[] = users.map(user => {
        ...
        const co2Saved = totalRecycled * 1.5;
        const level = this.calculateLevel(totalPoints);
        const achievements = this.achievements.filter(a => 
          this.checkAchievementUnlocked(a, totalPoints, totalRecycled, 0)
        ).length;
  ...
  private async calculateRecyclingStreak(userId: string): Promise<number> {
  ...
  private calculateLevel(points: number): string {
    ...
    if (points < 500) return 'Green Enthusiast';
    if (points < 1000) return 'Recycling Pro';
    if (points < 2500) return 'Eco Warrior';
    if (points < 5000) return 'Sustainability Master';
    if (points < 10000) return 'Environmental Champion';
    return 'Green Legend';
  }
  ...
  private getNextLevelPoints(currentPoints: number): number {
    ...
    if (currentPoints < 10000) return 10000;
    return 0; // Max level
  }
