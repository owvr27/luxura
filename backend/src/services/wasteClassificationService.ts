...
export class WasteClassificationService {
  private static instance: WasteClassificationService;
  
  public static getInstance(): WasteClassificationService {
  ...
  async classifyWaste(description: string, imageUrl?: string): Promise<WasteClassificationResult> {
  ...
  private ruleBasedClassification(description: string): WasteClassificationResult {
  ...
  async getEnvironmentalImpact(userId: string): Promise<{
  ...
  async getRecyclingRecommendations(userId: string): Promise<{
    category: string;
