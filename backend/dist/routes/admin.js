...
const router = Router();
// Dashboard metrics
router.get('/admin/dashboard', authRequired, adminOnly, async (_req, res) => {
...
router.post('/admin/bins', authRequired, adminOnly, async (req, res) => {
    const parsed = binSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid bin data' });
...
router.get('/admin/operations', authRequired, adminOnly, async (_req, res) => {
